...
function Decore:init( boardRig, rnd, decorInfo, orientation )	
	...
	if animdef.setlayer == "floor" then
		prop:setPeriphery( "data/images/los_full.png", "data/images/los_partial.png" )
	...
	self._optional = (animdef.boundType >= 3) and (animdef.boundType <= 7)
...
function Decore:refreshVisibility( cellx, celly, visStatus )
	...
	if currentVis == VIS_NONE or (self._optional and not gfxOptions.enableOptionalDecore) then
...
function Decore:refresh( orientation, gfxOptions )
	...
	for i = 1, #self._cells do
		...
		for obstruction,value in pairs( cellviz._obstruction_values ) do
	...
	uniforms:setUniformFloat( "FocusAmount", 1.0 )
...
function decorig:init( boardRig, levelData, params )
	...
	local rnd = rand.createGenerator( params.seed )
	local orientation = boardRig._game:getCamera():getOrientation()

	for i,decorInfo in ipairs( levelData.decos ) do
		local decor = Decore( boardRig, rnd, decorInfo, orientation )		
		local cells = decor:getCells()
		for i = 1, #cells do
...
function decorig:queryCellDecorFocusList( x, y )
	...
	for _,region in ipairs( focusRegion ) do
		...
		for _,decor in ipairs( decors ) do
			if not excluded[decor] then
				local mx, my, mz, MX, MY, MZ = decor._prop:getWorldBounds()
				if h <= (MZ - mz) / iso_dim or true then
					if not included[decor] then
						included[decor] = true
