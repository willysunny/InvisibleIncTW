...
function fxmanager:addMarkerFx( x, y, radius, duration, r, g, b, a )
	...
	scriptDeck:setDrawCallback ( 
		function( index, xOff, yOff, xFlip, yFlip )
			...
			MOAIDraw.fillCircle( x, y, radius, 12 )
		end )
	...
	local fx =
	{
		...
		followProp = function( self, parent )
			self._prop:setAttrLink(MOAIProp.INHERIT_TRANSFORM, parent, MOAIProp.TRANSFORM_TRAIT)
		end,
		...
		setLoop = function( self, isLoop )
			if isLoop then
				...
				self._prop:setListener( KLEIAnim.EVENT_ANIM_END,
					function( anim ) self._done = true end )
			end
		end,
...
function fxmanager:addFloatLabelToPoint( x0, y0, x1, y1, text ,duration,color, z, ts )

	--local y1 = y1 - 545
	--local x1 = x1 - 85
	local x2, y2 = self._layer:wndToWorld( x1, y1 )
	local prop = MOAITextBox.new()
	prop:setRect( -64, -64, 64, 64 )
	prop:setString(string.format(text))
	prop:setFont( self._font )
	...
	local easeDuration = math.max( 1, duration)
	prop:seekLoc( x2, y2, (z or 0), easeDuration, MOAIEaseType.SHARP_EASE_IN )
	prop:seekScl( 0.5, 0.5, 0.5, easeDuration, MOAIEaseType.SHARP_EASE_IN )
...
function fxmanager:widgetFlyTxt( x0, y0, x1, y1, text ,duration, screen, scale, color)
