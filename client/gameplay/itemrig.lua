...
function itemtex_rig:refreshRenderFilter()
	if self:getLocation() then
		...
		if cell then
			...
			if not cell.ghostID or gfxOptions.bFOWEnabled == false then
...
function itemtex_rig:onSimEvent( ev, eventType, eventData )
	...
	if eventType == simdefs.EV_UNIT_REFRESH then
		self:refresh()
		self:refreshRenderFilter()

	elseif eventType == simdefs.EV_UNIT_UNGHOSTED or eventType == simdefs.EV_UNIT_WARPED then
		self:refresh()
		self:refreshRenderFilter()			
	end
end
...
local _M = {}
function _M.createItemRig( boardRig, unit )
