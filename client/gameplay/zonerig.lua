...
local function drawMotionSense( boardRig, cell )
	local x0, y0 = boardRig:cellToWorld( cell.x, cell.y )

	MOAIGfxDevice.setPenColor(1, 0, 0, 0.2)
	MOAIDraw.fillCircle( x0, y0, 32*.38, 12 )
end

local function drawZones( self, boardRig )
	local simCore = boardRig._game.simCore
	local simquery = simCore.getQuery()
	local simdefs = simCore.getDefs()
	local w,h = simCore:getBoardSize()

	if simCore:getCurrentPlayer() == boardRig._game:getLocalPlayer() then
		for _, cell in ipairs( simCore:getMotionCells() ) do
			drawMotionSense( boardRig, cell )
		end
	end
	...
	for hiliteID, hilite in pairs(self._hilites) do
		...
		for i,x,y in util.xypairs(hilite.cells) do 
...
function zonerig:init( layer, board )
	...
	local ww, wh = board:getWorldSize()
...
--	assert( self._hilites[ hiliteID ] ~= nil )
...
function zonerig:setMovementTiles( tiles, grad, line)
	...
	self._curves = { createFadeCurve( self, grad.r ), createFadeCurve( self, grad.g ), createFadeCurve( self, grad.b ), createFadeCurve( self, grad.a ) }

	self._movementGradProp:setAttrLink ( MOAIColor.ATTR_R_COL, self._curves[1], MOAIAnimCurve.ATTR_VALUE )
	self._movementGradProp:setAttrLink ( MOAIColor.ATTR_G_COL, self._curves[2], MOAIAnimCurve.ATTR_VALUE )
	self._movementGradProp:setAttrLink ( MOAIColor.ATTR_B_COL, self._curves[3], MOAIAnimCurve.ATTR_VALUE )
	self._movementGradProp:setAttrLink ( MOAIColor.ATTR_A_COL, self._curves[4], MOAIAnimCurve.ATTR_VALUE )

	self._movementLineProp:setAttrLink ( MOAIColor.ATTR_R_COL, self._curves[1], MOAIAnimCurve.ATTR_VALUE )
	self._movementLineProp:setAttrLink ( MOAIColor.ATTR_G_COL, self._curves[2], MOAIAnimCurve.ATTR_VALUE )
	self._movementLineProp:setAttrLink ( MOAIColor.ATTR_B_COL, self._curves[3], MOAIAnimCurve.ATTR_VALUE )
	self._movementLineProp:setAttrLink ( MOAIColor.ATTR_A_COL, self._curves[4], MOAIAnimCurve.ATTR_VALUE )
end
