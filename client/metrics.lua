...
function level_finished:onSend( data, user, game, playTime )
	log:write( " ### Sending level_finished..." )

	data.event = "level_finished"

	local result
	if game.simCore:getWinner() then
		result = "win"
	elseif game.simCore:isGameOver() then
		result = "lose"
	else
		result = "incomplete"
	end

	data.game_stats = util.extend( game.simCore:getStats() )
	{
		credits = game.simCore:getPC():getCredits(),
		num_actions = #game.simHistory,
		num_turns = math.floor( game.simCore:getTurnCount() / 2 ),
		result = result,
		play_t = playTime
	}

	-- Don't bother including certain values for 'tutorial' levels.
	if game.params.world ~= nil then
		local agency = game.params.players[1].agency
		data.game_params =
		{
			game_number = user.data.num_games,
			seed = game.params.seed,
			progress = game.params.campaignHours,
			difficulty = game.params.difficulty,
			gameDifficulty = game.params.gameDifficulty,
			missionCount = game.params.missionCount,
			situationName = game.params.situationName,
			agency =
			{
				-- Note that at this point, agency does NOT have the updated stats from post-game.
				agents = {},
			},
			location = game.params.location,
			world = game.params.world,
		}
