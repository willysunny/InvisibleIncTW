...
local function setVisible( isVisible )
	...
	for i = #MUI._activeScreens,1,-1 do
		...
		if screen._filename ~= "debug-panel.lua" or isVisible then
...
local function deactivateScreen( screen )
	...
	if screen:hasTransition( screen:getProperties().deactivateTransition ) then
	...
	elseif screen:hasTransition( screen:getProperties().activateTransition ) then
