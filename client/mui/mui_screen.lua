...
function mui_screen:handlesInput()
	return self:isActive() and
			not self:hasTransition( self._props.deactivateTransition, MOAITimer.NORMAL ) and
			not self:hasTransition( self._props.activateTransition, MOAITimer.REVERSE )
end
...
function mui_screen:handleInputEvent( ev ) 
	...
	if tooltip == nil and self.onTooltip then
