...
function mui_tooltip:init( header, body, footer )
	self._headerTxt = header
	self._bodyTxt = body
	self._footerTxt = footer
end
...
function mui_tooltip:activate( screen )
	...
	if self._footerTxt then
		hotkeyLabel:setText( string.format( "HOTKEY: <tthotkey>%s</>", self._footerTxt ))
	else
	...
	local Y_FUDGE_FACTOR = 12 / H -- Not sure why this exists, perhaps getSTringBounds is inaccurate?
	...
	th = th + 2 * math.abs(y) - h + Y_FUDGE_FACTOR
	...
	if self._footerTxt then
		...
		footer:setPosition(W * tw / 2, H * (-th + math.abs(ymax_hotkey - ymin_hotkey)) +2  )
		hotkeyLabel:setPosition( nil, H * (-th + math.abs(ymax_hotkey - ymin_hotkey)) - 2 )
	else
