...
function mui_button:isClickButton( button )
	if self._clickButton then
		...
		return button == mui_defs.MB_Left or button == mui_defs.MB_Right
	end
end
...
function mui_button:handleEvent( ev )
	if ev.eventType == mui_defs.EVENT_KeyDown and ev.key == self._hotkey and self:isVisible() and not ev.controlDown and not ev.altDown and not ev.shiftDown then
		if self._buttonState ~= mui_button.BUTTON_Disabled then
			self:dispatchEvent( { eventType = mui_defs.EVENT_ButtonClick, widget = self, ie = ev } )
			return true
		end
