...
function updateImageState( btn, img )
		...
		self._knobBtn:setTrackHandler(
			function( dx, dy, ev )
				...
				if self._max > self._min then
					if self._orientation == ORIENT_H then
						dv = (dx / getTrackImageLength( self )) * (self._max - self._min)
					else
						dv = (-dy / getTrackImageLength( self )) * (self._max - self._min)
					end
				end

				if dv >= 1 or dv <= -1 then
