...
function mui_slider:init( screen, def )
	...
	self._trackImg = mui_texture( screen, { x = 0, y = 0, w = def.w, h = def.h, images = def.bg_image })
	self._trackBtn = mui_button( { x = 0, y = 0, w = def.w, h = def.h })
	self._trackBtn:addEventHandler( self, mui_defs.EVENT_ALL )
	
	self._knobImg = mui_texture( screen, { x = 0, y = 0, w = def.knobw, h = def.knobh, images = def.images })
	self._knobBtn = mui_trackbutton( { x = 0, y = 0, w = def.knobw, h = def.knobh })
