...
local cdefs = require( "client_defs" )
require( "class" )
...
function mui_text:init( screen, def )
	...
	if type(def.locstr) == "string" then
		-- NOTE:
		-- def.locstr is a sequence of table accesses that resolves to a string field if valid.
		-- Note that if not a string type, def.locstr is not necessarily nil (hence the form of the if condition).
		-- This is due to the tool pipeline: See uitool/main.lua for details.
		self:setText( def.locstr )
	elseif def.str then
		log:write( cdefs.LOG_LOC, "WARNING: %s.%s not localized '%s'", screen._filename, tostring(def.name), tostring(def.str) )
...
function mui_text:getStringBounds()
	local xmin, ymin, xmax, ymax = self._prop:getStringBounds()
